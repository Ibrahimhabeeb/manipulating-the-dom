...
form.addEventListener('submit', function (e) {
  // since it submits to server we need to prevent that
  e.preventDefault();
  // all inputs have a value but initially stored has a string
  const inputed = parseInt(amount.value);
  console.log(inputed);
  let ring = "";
  // when value is empty, lessthan 0 greater than 9
  const randnum = Math.floor(Math.random() * text.length);
  if (isNaN(inputed) || inputed < 0 || inputed > 9) {
    result.innerHTML = `<p class="text">${text[randnum]}</p>`
  }
  else {
    for (i = 0; i < inputed; i++) {
      ring += `<p class="text">${text[i]}</p>`;
      // console.log(ring);
